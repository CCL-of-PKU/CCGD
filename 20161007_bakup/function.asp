...
NUM_PER_PAGE = 8    '列表每页显示个数

FEATURE_VALUE_ARRAY = Array("异序","错配","省略","复现","冗余","论元异常")
TYPE_VALUE_ARRAY = Array("半凝固型","凝固型","短语型","复句型")
  ...
  if not (rs.EOF or rs.BOF) then
    ...
    <table class='table table-bordered table-hover'>
      <thead><tr>
        ...
        <th width="10%">构式类型</th>
        <th width="10%">构式特征</th>
        <th width="6%" style="text-align:center">变项数量</th>
        <th width="6%" style="text-align:center">常项数量</th>
        <th width="30%">实例</th>
    ...
    while not (rs.EOF or count > rs.pagesize)
      ...
      <tr>
        ...
        <td><%=rs("type")%></td>
        <td><%=rs("feature")%></td>
        <td style="text-align:center"><%=rs("variables")%></td>
        <td style="text-align:center"><%=rs("constants")%></td>
        <td><%=rs("example")%></td>
...
' 判断form构式形式是否已经存在
'--------------------------
function isExistedForm(str)
	newsql = "select top 1 ID from construction where form = '" & str & "'"
    Set rs = Server.CreateObject("adodb.recordset")
	rs.open newsql,Conn,1,1
