...
FEATURE_VALUE_ARRAY = Array("异序","语法错配","省略","复现","冗余","论元异常","含否定成分","含疑问成分","否定义","负面评价","周遍","主观大量","主观小量","语义错配","修辞", "网络用语")  'Hybin 2018-09-24
...
'只有碰到多义构式时，才需要增加相同的构式形式的记录
	...
	if (Construction_Form_Checked = false) then
		...
		call ClientScript.ClientScript.RegisterOnSubmitStatement(Me.GetType(), "preadd", script)
	end if
